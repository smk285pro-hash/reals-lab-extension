...
void TDStretch::acceptNewOverlapLength(int newOverlapLength)
{
    ...
    if (overlapLength > prevOvl)
    {
        ...
        // Allocate with extra 64-element padding to prevent SIMD/SSE unrolled read overruns
        pMidBufferUnaligned = new SAMPLETYPE[overlapLength * channels + 64];
...
void TDStretch::calculateOverlapLength(int overlapInMsec)
{
    ...
    // must be divisible by 16 for SSE vectorization
    newOvl -= newOvl % 16;
    if (newOvl < 16) newOvl = 16;
